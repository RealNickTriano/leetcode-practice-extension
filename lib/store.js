...
(function () {
  ...
  if (typeof module !== "undefined" && module.exports) {
    ...
    // Chrome's background is a service worker (no DOM), so the absence of
    // `document` identifies it. Firefox's background is an event page — a hidden
    // DOM document — so that test fails there; lib/bg-context.js sets an
    // explicit flag, loaded ahead of this file in the Firefox manifest.
    const isBackgroundContext =
      typeof document === "undefined" || globalThis.__LEETCODE_ANKI_BACKGROUND__;

    if (isBackgroundContext) {
      // Background context: the one real store. Every write in the
